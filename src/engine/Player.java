...
public class Player {
	...
	public String getText(int x) {
		String z="Definding Army :";
		for(int j=0;j<this.getControlledCities().get(x).getDefendingArmy().getUnits().size();) {
			String c="";
			String type="";
			Unit unit=this.getControlledCities().get(x).getDefendingArmy().getUnits().get(j);
		...
		for(int i=0;i<this.getControlledArmies().size();i++) {
			z+="Army "+(i+1);
			for(int j=0;j<this.getControlledArmies().get(i).getUnits().size();) {
				String c="";
				String type="";
				Unit unit= this.getControlledArmies().get(i).getUnits().get(j);
				if(unit instanceof Archer) type="Archer";
				else if(unit instanceof Cavalry) type="Cavalry";
				else type="Infantry";
				c="(Unit "+ (++j)+" :- Type : "+type+", Level : "+unit.getLevel()+", Current Solider Count : "+ unit.getCurrentSoldierCount()+", Max Soldier Count : "+ unit.getMaxSoldierCount()+") ";
